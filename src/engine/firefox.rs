...
impl FirefoxEngine {
    pub async fn launch(options: LaunchOptions) -> Result<Self, TurbosheetError> {
        ...
        let process = cmd.spawn().map_err(|e| {
            TurbosheetError::LaunchFailed(format!("Failed to spawn geckodriver: {}", e))
        })?;
...
impl BrowserEngine for FirefoxEngine {
    ...
    async fn new_context(
        ...
        let bidi_client = {
            ...
            let session_id = client.session_id.clone().unwrap_or_default();
            let base_url = client.base_url.clone();
            if !session_id.is_empty() {
                match BidiClient::from_webdriver_session(&base_url, &session_id).await {
                    Ok(bc) => Some(bc),
                    Err(e) => {
                        tracing::warn!("BiDi connect failed (non-fatal): {}", e);
                        None
                    }
                }
            } else {
        ...
        Ok(Arc::new(FirefoxContextEngine {
...
#[cfg(feature = "bidi")]
use crate::engine::bidi::client::BidiClient;

pub struct FirefoxContextEngine {
    client: Arc<tokio::sync::Mutex<WebDriverClient>>,
    pages: Arc<tokio::sync::RwLock<Vec<Arc<dyn PageEngine>>>>,
    #[cfg(feature = "bidi")]
    bidi_client: Option<Arc<BidiClient>>,
}

#[async_trait]
impl ContextEngine for FirefoxContextEngine {
    async fn new_page(&self) -> Result<Arc<dyn PageEngine>, TurbosheetError> {
        ...
        let engine = Arc::new(FirefoxPageEngine { 
            client: self.client.clone(),
            global_name,
            binding_name,
            ...
            #[cfg(feature = "bidi")]
            bidi_client: self.bidi_client.clone(),
        });
    ...
    async fn cookies(&self, urls: Option<Vec<String>>) -> Result<Vec<crate::context::Cookie>, TurbosheetError> {
        ...
        // Parse the WebDriver response: { value: [ { name, value, domain, path, ... } ] }
        ...
        if let Some(url_list) = urls {
            cookies.retain(|cookie| {
                url_list.iter().any(|url| {
                    cookie_matches_url(cookie, url)
                })
            });
        }
...
#[async_trait]
impl PageEngine for FirefoxPageEngine {
    ...
    async fn close(&self) -> Result<(), TurbosheetError> {
        // Handled by context/browser closing in standard WebDriver, or we can close the window
        let client = self.client.lock().await;
    ...
    async fn wait_for_request(&self, url: &str) -> Result<(), TurbosheetError> {
        #[cfg(feature = "bidi")]
        if let Some(ref bidi) = self.bidi_client {
            let _ = bidi.wait_for_request(url).await?;
            return Ok(());
        }
    ...
    async fn wait_for_response(&self, url: &str) -> Result<(), TurbosheetError> {
        #[cfg(feature = "bidi")]
        if let Some(ref bidi) = self.bidi_client {
            let _ = bidi.wait_for_response(url).await?;
            return Ok(());
        }
    ...
    async fn evaluate_handle(&self, _js: &str) -> Result<(), TurbosheetError> { Ok(()) }
    ...
    async fn expose_function(&self, name: &str, js_body: &str) -> Result<(), TurbosheetError> {
        ...
        #[cfg(feature = "bidi")]
        if let Some(ref bidi) = self.bidi_client {
            let _ = bidi.add_preload_script(&js).await?;
            return Ok(());
        }
